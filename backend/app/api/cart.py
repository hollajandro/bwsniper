...
from ..services.buywander_api import (
    create_bw_session, validate_session,
    fetch_cart_and_visits, fetch_reserved_auctions, fetch_payment_methods,
    fetch_open_slots, fetch_removal_status,
    do_pay_checkout, do_create_appointment, do_reschedule_appointment,
...
def _resolve(db: Session, user: User, login_id: str):
    ...
    if not validate_session(bw):
        log.warning("BW session invalid for login %s — re-authenticating", login.bw_email)
        try:
            bw = reauth_bw_login(login, db)
            log.warning("Re-authentication succeeded for %s", login.bw_email)
        except Exception as ex:
            log.error("Re-authentication failed for %s: %s", login.bw_email, ex, exc_info=True)
            raise HTTPException(
                status_code=502,
...
def get_cart(login_id: str,
    ...
    try:
        with ThreadPoolExecutor(max_workers=3) as executor:
            cart_future = executor.submit(fetch_cart_and_visits, bw, cid, store_location_id)
...
def get_open_slots(login_id: str,
...
def get_removal_status(login_id: str,
                       store_location_id: str = Query(...),
                       user: User = Depends(get_current_user),
                       db: Session = Depends(get_db)):
...
def create_appointment(login_id: str,
...
def reschedule_appointment(login_id: str, visit_id: str,
...
def cancel_appointment(login_id: str, visit_id: str,
...
def remove_cart_item(login_id: str,
                     req: CartRemoveItem,
                     user: User = Depends(get_current_user),
                     db: Session = Depends(get_db)):
    bw, _ = _resolve(db, user, login_id)
