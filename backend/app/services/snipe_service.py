...
def create_snipe(db: DBSession, user_id: str, login_id: str,
                 url: str, bid_amount: float, snipe_seconds: int,
...
def update_snipe(db: DBSession, user_id: str, snipe_id: str,
                 update_data: dict = None) -> Snipe:
    ...
    """
    snipe = db.query(Snipe).join(BuyWanderLogin).filter(
        Snipe.id == snipe_id,
        BuyWanderLogin.user_id == user_id,
    ).first()
    if not snipe:
...
def get_user_snipes(db: DBSession, user_id: str,
                    login_id: str = None,
                    include_deleted: bool = False) -> list[Snipe]:
    """Return all snipes belonging to the user, optionally filtered."""
    q = db.query(Snipe).join(BuyWanderLogin).filter(
        BuyWanderLogin.user_id == user_id)
    if login_id:
...
def restart_active_snipes(db: DBSession, ws_manager=None):
    ...
    """
    snipes = db.query(Snipe).filter(
        Snipe.status.in_(list(SnipeStatus.active()))).all()
