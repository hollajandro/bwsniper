...
from ..services.snipe_service import (
    create_snipe, update_snipe, delete_snipe, get_user_snipes,
)
...
def add_snipe(request: Request,
    ...
    try:
        snipe = create_snipe(
            db, user.id, req.login_id,
            req.url, req.bid_amount, req.snipe_seconds,
...
def list_snipes(login_id: Optional[str] = Query(None),
                user: User = Depends(get_current_user),
                db: Session = Depends(get_db)):
